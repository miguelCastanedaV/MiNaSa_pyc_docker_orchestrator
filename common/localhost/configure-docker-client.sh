...
show_instructions() {
    ...
    echo "  docker pull $FULL_REGISTRY/gs-phva-web:latest"
    echo "  docker pull $FULL_REGISTRY/gs-phva-worker:v1.0.0"
    echo ""
    echo -e "${YELLOW}Para usar en docker-compose.yml:${NC}"
    echo "  services:"
    echo "    phva:"
    echo "      image: \${REGISTRY_URL:-$FULL_REGISTRY}/gs-phva-web:\${APP_VERSION:-latest}"
    echo ""
    ...
    echo "  export IMAGE_WEB=\${REGISTRY_URL}/\${REGISTRY_NAMESPACE}/gs-phva-web"
    echo "  export IMAGE_WORKER=\${REGISTRY_URL}/\${REGISTRY_NAMESPACE}/gs-phva-worker"
}
